...
ID3D11Buffer* g_DrawModeBuffer = NULL; // 追加
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	td.Width = sd.BufferDesc.Width; td.Height = sd.BufferDesc.Height;
	...
	dsvd.Format = td.Format; dsvd.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
	...
	g_D3DDevice->CreateDepthStencilState(&dsd, &g_DepthStateEnable);
	dsd.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ZERO;
	g_D3DDevice->CreateDepthStencilState(&dsd, &g_DepthStateDisable);
	...
	if (FAILED(hr)) {
		if (pErrorBlob) {
			MessageBoxA(nullptr, (char*)pErrorBlob->GetBufferPointer(), "VS Compile Error", MB_OK | MB_ICONERROR);
			pErrorBlob->Release();
		}
		else {
			MessageBoxA(nullptr, "shader.hlsl が見つからない、または読み込めません。", "VS Compile Error", MB_OK | MB_ICONERROR);
		}
		return hr;
	}
	...
	g_D3DDevice->CreateInputLayout(layout, ARRAYSIZE(layout),
	...
	if (FAILED(hr)) {
		MessageBoxA(NULL, (char*)pErrorBlob->GetBufferPointer(), "PS Compile Error", MB_OK | MB_ICONERROR);
		return hr;
	}
	...
	hbd.Usage = D3D11_USAGE_DEFAULT; hbd.BindFlags = D3D11_BIND_CONSTANT_BUFFER; hbd.StructureByteStride = sizeof(float);

	// b0: WVP (VS)
	hbd.ByteWidth = sizeof(XMMATRIX);
	g_D3DDevice->CreateBuffer(&hbd, NULL, &g_ConstantBuffer);
	g_ImmediateContext->VSSetConstantBuffers(0, 1, &g_ConstantBuffer);

	// b1: Material (VS)
	hbd.ByteWidth = sizeof(MATERIAL);
	g_D3DDevice->CreateBuffer(&hbd, NULL, &g_MaterialBuffer);
	...
	g_D3DDevice->CreateBuffer(&hbd, &initData, &g_DrawModeBuffer);
